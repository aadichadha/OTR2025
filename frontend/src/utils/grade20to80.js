/**
 * 20-80 Baseball Scouting Grade System - Frontend Utilities
 ...
 */
export function calculateGrade(value, mean, sd) {
 ...
 */
export function calculateGradeChange(oldValue, newValue, mean, sd) {
  const oldGrade = calculateGrade(oldValue, mean, sd);
  const newGrade = calculateGrade(newValue, mean, sd);
 ...
 */
export function getCoachingTip(metric, currentGrade, targetGrade) {
  const tips = {
    'avgEv': {
 ...
 */
export function getMilestones(metric, mean, sd) {
  const milestones = [
